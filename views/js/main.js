// NOTE: I have made several changes to optimize the Javascript for performance. See comments on lines 346, 414, 435, 474, 505 and 519
...
// Rather than construct the random pizza containers one element at a time, I have created a single HTML string (so that the browser only has to modify the DOM once per random pizza)
var pizzaElementGenerator = function(i) {

    return '<div id="pizza' + i + '" class="randomPizzaContainer" style="width:33.33%; min-height: 350px;">' +
        '<div class="col-md-6">' +
...
// Here, I have changes the code so that the DOM is updated only once, after all the random pizza containers are strung together. 

var randomPizzaHolder = document.getElementById("randomPizzas");
var allPizzasHTML = "";

for (var i = 2; i < 100; i++) {
  allPizzasHTML += pizzaElementGenerator(i);
}

randomPizzaHolder.innerHTML = allPizzasHTML;
...
function updatePositions() {
  ...
  // Added this code to ensure that the event is actually a scroll; if it's not, then we set scrollTop to 0
...
function shiftPizzas(scrollTop) {
...
// Changed the number of iterations from 200 to 32. Since the pizzas are in a fixed position in the browser, it is not necessary to have more than 5 rows of 8 pizzas. 
document.addEventListener('DOMContentLoaded', function(event) {
  ...
  updatePositions(event); // Added "event" so that updatePositions can validate the event's type
});
